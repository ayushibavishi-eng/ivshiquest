...
describe("Grade 6 Ganita Prakash Math lesson batch", () => {
  it("authors one complete Grade 6 lesson for each Ganita Prakash chapter", () => {
    const world = getCurriculumWorld("ncert-g6-ganita-prakash");
    assert.ok(world, "Ganita Prakash Grade 6 world should exist");
    assert.equal(world.curriculumId, "india-current");
    assert.equal(world.topics.length, 10);
    assert.equal(world.concepts.length, 10);
    assert.equal(GRADE_6_MATH_CONCEPT_IDS.length, 10);
    assert.equal(GRADE_6_MATH_LESSONS.length, 10);

    for (const concept of world.concepts) {
      assert.ok(
        GRADE_6_MATH_CONCEPT_IDS.includes(
          concept.id as (typeof GRADE_6_MATH_CONCEPT_IDS)[number],
        ),
        `unexpected Ganita Prakash concept ${concept.id}`,
      );
  ...
  it("keeps Grade 6 India Math catalogue filtered from empty USA Math", () => {
    const india = getCurriculumWorldsForSubject("math", 6, "india-current");
    const usa = getCurriculumWorldsForSubject("math", 6, "us");
    assert.ok(india.some((world) => world.id === "ncert-g6-ganita-prakash"));
    assert.equal(usa.length, 0);
  });

  it("preserves gold-standard Equivalent Fractions and other gold lessons", () => {
  ...
  it("gives every Grade 6 Math lesson the complete-lesson shape", () => {
    const seenQuestionIds = new Set<string>();

    for (const definition of GRADE_6_MATH_LESSONS) {
  ...
  it("chains the 10 chapters in Ganita Prakash order", () => {
    for (let index = 0; index < GRADE_6_MATH_LESSONS.length; index += 1) {
      const lesson = GRADE_6_MATH_LESSONS[index];
      const expectedId = GRADE_6_MATH_CONCEPT_IDS[index];
      assert.equal(lesson.conceptId, expectedId);

      if (index === 0) {
        assert.deepEqual(lesson.prerequisites, []);
      } else {
        assert.ok(lesson.prerequisites.includes(GRADE_6_MATH_CONCEPT_IDS[index - 1]));
      }

      if (index === GRADE_6_MATH_LESSONS.length - 1) {
        assert.deepEqual(lesson.nextConceptIds, []);
      } else {
        assert.deepEqual(lesson.nextConceptIds, [
          GRADE_6_MATH_CONCEPT_IDS[index + 1],
        ]);
      }
    }
  });

  it("accepts reasonable written answers on sample Grade 6 prompts", () => {
    const patterns = getCompleteLesson(
      "ncert-g6-ganita-prakash-patterns-in-mathematics-core",
      6,
    );
    assert.ok(patterns);
    const write =
      patterns.content.practice.find((item) => item.format === "write") ??
      patterns.content.reasoning.find((item) => item.format === "write");
    assert.ok(write);
  ...
  it("maps every Grade 6 math skill-spine concept onto a host lesson", () => {
    const audit = grade6MathSpineCoverageAudit();
    assert.equal(audit.spineItems, 72);
    assert.equal(audit.taughtCount + audit.overlapCount, 72);
    assert.equal(new Set(audit.connectedIds).size, 72);

    for (const row of GRADE_6_MATH_SPINE_COVERAGE) {
  ...
  it("makes the full Grade 6 math catalogue learnable without extra lessons", () => {
    const mathWorlds = getCurriculumWorldsForSubject("math", 6);
    ...
    for (const world of mathWorlds) {
      for (const concept of getWorldPathForGrade(world, 6)) {
        ...
        assert.ok(
          ...
          `Grade 6 math concept ${concept.id} still has no learning content`,
        );
      }
    }
    assert.equal(conceptIds.length, 83);

    const tree = getKnowledgeTreeGraph("math", 6);
