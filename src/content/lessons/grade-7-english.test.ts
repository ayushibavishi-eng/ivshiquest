...
describe("Grade 7 Poorvi English lesson batch", () => {
  it("authors one complete Grade 7 lesson for each official Poorvi text", () => {
    const world = getCurriculumWorld("ncert-g7-poorvi");
    assert.ok(world, "Poorvi Grade 7 world should exist");
    assert.equal(world.concepts.length, 15);
    assert.equal(GRADE_7_ENGLISH_CONCEPT_IDS.length, 15);
    assert.equal(GRADE_7_ENGLISH_LESSONS.length, 15);

    for (const concept of world.concepts) {
      assert.ok(
        GRADE_7_ENGLISH_CONCEPT_IDS.includes(
          concept.id as (typeof GRADE_7_ENGLISH_CONCEPT_IDS)[number],
        ),
  ...
  it("gives every Grade 7 English lesson the complete-lesson shape", () => {
    const seenQuestionIds = new Set<string>();

    for (const definition of GRADE_7_ENGLISH_LESSONS) {
  ...
  it("chains the 15 texts in Poorvi order", () => {
    for (let index = 0; index < GRADE_7_ENGLISH_LESSONS.length; index += 1) {
      const lesson = GRADE_7_ENGLISH_LESSONS[index];
      const expectedId = GRADE_7_ENGLISH_CONCEPT_IDS[index];
      ...
      if (index === 0) {
        ...
        assert.ok(
          lesson.prerequisites.includes(GRADE_7_ENGLISH_CONCEPT_IDS[index - 1]),
        );
      }

      if (index === GRADE_7_ENGLISH_LESSONS.length - 1) {
        assert.deepEqual(lesson.nextConceptIds, []);
      } else {
        assert.deepEqual(lesson.nextConceptIds, [
          GRADE_7_ENGLISH_CONCEPT_IDS[index + 1],
        ]);
      }
    }
  });

  it("accepts reasonable written answers on sample Grade 7 English prompts", () => {
    const river = getCompleteLesson(
      "ncert-g7-poorvi-the-day-the-river-spoke-core",
      7,
    );
    assert.ok(river);
    const write =
      river.content.practice.find((item) => item.format === "write") ??
      river.content.reasoning.find((item) => item.format === "write") ??
      river.content.retrieve.find((item) => item.format === "write");
    assert.ok(write);
  ...
  it("maps every Grade 7 English skill-spine concept onto a host lesson", () => {
    const audit = grade7EnglishSpineCoverageAudit();
    assert.equal(audit.spineItems, 25);
    assert.equal(audit.taughtCount + audit.overlapCount, 25);
    assert.equal(new Set(audit.connectedIds).size, 25);
    assert.equal(
      GRADE_7_ENGLISH_SPINE_COVERAGE.some((row) => row.id === "english-sva-simple"),
      false,
    );

    for (const row of GRADE_7_ENGLISH_SPINE_COVERAGE) {
  ...
  it("makes the full Grade 7 English catalogue learnable without extra lessons", () => {
    const englishWorlds = getCurriculumWorldsForSubject("english", 7);
    ...
    for (const world of englishWorlds) {
      for (const concept of getWorldPathForGrade(world, 7)) {
        ...
        assert.ok(
          ...
          `Grade 7 English concept ${concept.id} still has no learning content`,
        );
      }
    }
    assert.equal(conceptIds.length, 40);

    const tree = getKnowledgeTreeGraph("english", 7);
