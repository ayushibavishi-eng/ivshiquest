...
describe("Grade 7 Curiosity Science lesson batch", () => {
  it("authors one complete Grade 7 lesson for each Curiosity chapter", () => {
    const world = getCurriculumWorld("ncert-g7-curiosity");
    assert.ok(world, "Curiosity Grade 7 world should exist");
    assert.equal(world.concepts.length, 12);
    assert.equal(GRADE_7_SCIENCE_CONCEPT_IDS.length, 12);
    assert.equal(GRADE_7_SCIENCE_LESSONS.length, 12);

    for (const concept of world.concepts) {
      assert.ok(
        GRADE_7_SCIENCE_CONCEPT_IDS.includes(
          concept.id as (typeof GRADE_7_SCIENCE_CONCEPT_IDS)[number],
        ),
  ...
  it("preserves Photosynthesis gold and prior science batches", () => {
    ...
    assert.equal(photo6.content.explore.activity.investigation?.kind, "plant-kitchen");
    ...
    assert.ok(
      getCompleteLesson("ncert-g6-curiosity-diversity-living-core", 6),
    );
    assert.ok(getCompleteLesson("ncert-g7-ganita-prakash-1-large-numbers-core", 7));
    for (const id of GOLD_STANDARD_CONCEPT_IDS) {
      assert.ok(getCurriculumConcept(id), `catalogue missing gold concept ${id}`);
    }
  });

  it("gives every Grade 7 Science lesson the complete-lesson shape", () => {
    const seenQuestionIds = new Set<string>();

    for (const definition of GRADE_7_SCIENCE_LESSONS) {
  ...
  it("chains the 12 chapters in Curiosity order", () => {
    for (let index = 0; index < GRADE_7_SCIENCE_LESSONS.length; index += 1) {
      const lesson = GRADE_7_SCIENCE_LESSONS[index];
      const expectedId = GRADE_7_SCIENCE_CONCEPT_IDS[index];
      ...
      if (index === 0) {
        ...
        assert.ok(
          lesson.prerequisites.includes(GRADE_7_SCIENCE_CONCEPT_IDS[index - 1]),
        );
      }

      if (index === GRADE_7_SCIENCE_LESSONS.length - 1) {
        assert.deepEqual(lesson.nextConceptIds, []);
      } else {
        assert.deepEqual(lesson.nextConceptIds, [
          GRADE_7_SCIENCE_CONCEPT_IDS[index + 1],
        ]);
      }
    }
  });

  it("accepts reasonable written answers on sample Grade 7 Science prompts", () => {
    const ever = getCompleteLesson("ncert-g7-curiosity-ever-evolving-core", 7);
    assert.ok(ever);
    const write =
      ever.content.practice.find((item) => item.format === "write") ??
      ever.content.reasoning.find((item) => item.format === "write") ??
      ever.content.retrieve.find((item) => item.format === "write");
    assert.ok(write);
  ...
  it("maps every Grade 7 science skill-spine concept onto a host lesson", () => {
    const audit = grade7ScienceSpineCoverageAudit();
    assert.equal(audit.spineItems, 12);
    assert.equal(audit.taughtCount + audit.overlapCount, 12);
    assert.equal(new Set(audit.connectedIds).size, 12);
    assert.equal(
      GRADE_7_SCIENCE_SPINE_COVERAGE.some(
        (row) => row.id === "science-plants-photosynthesis",
      ),
      false,
    );

    for (const row of GRADE_7_SCIENCE_SPINE_COVERAGE) {
  ...
  it("makes the full Grade 7 science catalogue learnable without extra lessons", () => {
    const scienceWorlds = getCurriculumWorldsForSubject("science", 7);
    ...
    for (const world of scienceWorlds) {
      for (const concept of getWorldPathForGrade(world, 7)) {
        ...
        assert.ok(
          ...
          `Grade 7 science concept ${concept.id} still has no learning content`,
        );
      }
    }
    assert.equal(conceptIds.length, 24);

    const tree = getKnowledgeTreeGraph("science", 7);
