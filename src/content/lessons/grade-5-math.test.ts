...
import {
  ...
  GRADE_5_MATH_MELA_CONCEPT_IDS,
  GRADE_5_MATH_SPINE_COVERAGE,
  grade5MathSpineCoverageAudit,
} from "@/content/lessons/grade-5-math";
...
describe("Grade 5 Maths Mela lesson batch", () => {
  it("authors one complete Grade 5 lesson for each Maths Mela chapter", () => {
    const world = getCurriculumWorld("ncert-g5-maths-mela");
    assert.ok(world, "Maths Mela Grade 5 world should exist");
    assert.equal(world.topics.length, 15);
    assert.equal(world.concepts.length, 15);
    assert.equal(GRADE_5_MATH_MELA_CONCEPT_IDS.length, 15);
    assert.equal(GRADE_5_MATH_LESSONS.length, 15);

    for (const concept of world.concepts) {
      assert.ok(
        GRADE_5_MATH_MELA_CONCEPT_IDS.includes(
          concept.id as (typeof GRADE_5_MATH_MELA_CONCEPT_IDS)[number],
        ),
  ...
  it("keeps gold-standard lessons and Grade 4 batches available", () => {
  ...
  it("gives every Grade 5 Math lesson the complete-lesson shape", () => {
    const seenQuestionIds = new Set<string>();

    for (const definition of GRADE_5_MATH_LESSONS) {
  ...
  it("chains the 15 chapters in Maths Mela order", () => {
    for (let index = 0; index < GRADE_5_MATH_LESSONS.length; index += 1) {
      const lesson = GRADE_5_MATH_LESSONS[index];
      const expectedId = GRADE_5_MATH_MELA_CONCEPT_IDS[index];
      assert.equal(lesson.conceptId, expectedId);

      if (index === 0) {
        assert.deepEqual(lesson.prerequisites, []);
      } else {
        assert.ok(lesson.prerequisites.includes(GRADE_5_MATH_MELA_CONCEPT_IDS[index - 1]));
      }

      if (index === GRADE_5_MATH_LESSONS.length - 1) {
        assert.deepEqual(lesson.nextConceptIds, []);
      } else {
        assert.deepEqual(lesson.nextConceptIds, [
          GRADE_5_MATH_MELA_CONCEPT_IDS[index + 1],
        ]);
      }
    }
  });

  it("accepts reasonable written answers on sample Grade 5 prompts", () => {
    const travellers = getCompleteLesson(
      "ncert-g5-maths-mela-we-the-travellers-1-core",
      5,
    );
    assert.ok(travellers);
    const write = travellers.content.practice.find((item) => item.format === "write");
    assert.ok(write);
    if (write.acceptedAnswers.length > 0) {
      assert.equal(writeAnswerSucceeds(write.acceptedAnswers[0], write), true);
    } else {
      assert.equal(
        writeAnswerSucceeds(write.keyIdeas.join(" "), write) ||
          writeAnswerSucceeds("thousand place estimate", write),
        true,
      );
    }

    const dairy = getCompleteLesson("ncert-g5-maths-mela-the-dairy-farm-core", 5);
    assert.ok(dairy);
    const dairyWrite = dairy.content.reasoning.find((item) => item.format === "write")
      ?? dairy.content.practice.find((item) => item.format === "write");
    assert.ok(dairyWrite);

    const data = getCompleteLesson(
  ...
  it("maps every Grade 5 math skill-spine concept onto a host lesson", () => {
    const audit = grade5MathSpineCoverageAudit();
    assert.equal(audit.spineItems, 91);
    assert.equal(audit.taughtCount + audit.overlapCount, 91);
    assert.equal(new Set(audit.connectedIds).size, 91);
    ...
    for (const row of GRADE_5_MATH_SPINE_COVERAGE) {
  ...
  it("makes the full Grade 5 math catalogue learnable without extra lessons", () => {
    const mathWorlds = getCurriculumWorldsForSubject("math", 5);
    ...
    for (const world of mathWorlds) {
      for (const concept of getWorldPathForGrade(world, 5)) {
        ...
        assert.ok(
          ...
          `Grade 5 math concept ${concept.id} still has no learning content`,
        );
      }
    }
    assert.equal(conceptIds.length, 107);

    const tree = getKnowledgeTreeGraph("math", 5);
