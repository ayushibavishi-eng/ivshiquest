...
describe("Grade 5 Our Wondrous World EVS lesson batch", () => {
  it("authors one complete Grade 5 lesson for each official EVS chapter", () => {
    const world = getCurriculumWorld("ncert-g5-wondrous-world");
    assert.ok(world, "Our Wondrous World Grade 5 world should exist");
    assert.equal(world.topics.length, 10);
    assert.equal(world.concepts.length, 10);
    assert.equal(GRADE_5_EVS_CONCEPT_IDS.length, 10);
    assert.equal(GRADE_5_EVS_LESSONS.length, 10);

    for (const concept of world.concepts) {
      assert.ok(
        GRADE_5_EVS_CONCEPT_IDS.includes(
          concept.id as (typeof GRADE_5_EVS_CONCEPT_IDS)[number],
        ),
      ...
      const catalogueSkills = concept.skills.map((skill) => skill.id);
      for (const skillId of catalogueSkills) {
        assert.ok(
          lesson.coveredSkillIds.includes(skillId),
          `${concept.id} should cover catalogue skill ${skillId}`,
        );
      }
    }
  });
  ...
  it("keeps gold-standard and prior batches available", () => {
    ...
    assert.ok(
      getCompleteLesson("ncert-g4-wondrous-world-nature-trail-core", 4),
    );
    assert.ok(
  ...
  it("gives every Grade 5 EVS lesson the complete-lesson shape", () => {
    const seenQuestionIds = new Set<string>();

    for (const definition of GRADE_5_EVS_LESSONS) {
  ...
  it("chains the 10 chapters in Our Wondrous World order", () => {
    for (let index = 0; index < GRADE_5_EVS_LESSONS.length; index += 1) {
      const lesson = GRADE_5_EVS_LESSONS[index];
      const expectedId = GRADE_5_EVS_CONCEPT_IDS[index];
      assert.equal(lesson.conceptId, expectedId);

      if (index === 0) {
        assert.deepEqual(lesson.prerequisites, []);
      } else {
        assert.ok(lesson.prerequisites.includes(GRADE_5_EVS_CONCEPT_IDS[index - 1]));
      }

      if (index === GRADE_5_EVS_LESSONS.length - 1) {
        assert.deepEqual(lesson.nextConceptIds, []);
      } else {
        assert.deepEqual(lesson.nextConceptIds, [
          GRADE_5_EVS_CONCEPT_IDS[index + 1],
        ]);
      }
    }
  });

  it("accepts reasonable written answers on sample Grade 5 EVS prompts", () => {
    const water = getCompleteLesson(
      "ncert-g5-wondrous-world-water-essence-core",
      5,
    );
    assert.ok(water);
    const write = water.content.practice.find((item) => item.format === "write")
      ?? water.content.reasoning.find((item) => item.format === "write");
    assert.ok(write);
  ...
  it("maps every Grade 5 science skill-spine concept onto a host lesson", () => {
    const audit = grade5ScienceSpineCoverageAudit();
    assert.equal(audit.spineItems, 76);
    assert.equal(audit.taughtCount + audit.overlapCount, 76);
    assert.equal(new Set(audit.connectedIds).size, 76);

    for (const row of GRADE_5_SCIENCE_SPINE_COVERAGE) {
  ...
  it("makes the full Grade 5 science catalogue learnable without extra lessons", () => {
    const scienceWorlds = getCurriculumWorldsForSubject("science", 5);
    ...
    for (const world of scienceWorlds) {
      for (const concept of getWorldPathForGrade(world, 5)) {
        ...
        assert.ok(
          ...
          `Grade 5 science concept ${concept.id} still has no learning content`,
        );
      }
    }
    assert.equal(conceptIds.length, 87);

    const tree = getKnowledgeTreeGraph("science", 5);
